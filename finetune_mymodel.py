...
parser = argparse.ArgumentParser()  # 创建一个新的参数解析器
parser.add_argument("--data_type", type=str, default="GAARA_RRAP_15")  # 定义数据类型参数
parser.add_argument("--log", type=str, default="logs")  
parser.add_argument("--wandb", action="store_true", default=False)  # 定义一个开关参数，用于控制是否使用Weights & Biases
parser.add_argument("--model_path", type=str, default="./models/vicuna")  
parser.add_argument("--model", type=str, default="vicuna")  
parser.add_argument("--eval_steps", type=int, default=200)  # 评估步数
parser.add_argument("--save_steps", type=int, default=200)  # 保存步数
parser.add_argument("--lr_scheduler_type", type=str, default="linear")  # 学习率调度器类型
parser.add_argument("--per_device_eval_batch_size", type=int, default=1)  
parser.add_argument("--total_batch_size", type=int, default=256)  # 总批次大小
parser.add_argument("--train_size", type=int, default=15942)  # 训练集大小 1ms数据集最小是8192
parser.add_argument("--val_size", type=int, default=1000)  # 验证集大小
parser.add_argument("--resume_from_checkpoint", type=str, default=None)  # 定义从哪个检查点恢复训练，默认不从检查点恢复
parser.add_argument("--lora_remote_checkpoint", type=str, default=None)  # 定义LoRA远程检查点
parser.add_argument("--ignore_data_skip", type=str, default="False")  # 是否忽略数据跳过
parser.add_argument("--lr", type=float, default=1e-3)  # 学习率
parser.add_argument("--wd", type=float, default=0)  # 权重衰减系数
parser.add_argument("--use_lora", type=int, default=1)  # 是否使用LoRA
parser.add_argument("--seed", type=int, default=42)  
parser.add_argument("--epochs", type=int, default=1)  # 训练轮数


parser.add_argument("--dataset", type=str, default="BookCrossing")  # 数据集名称
parser.add_argument("--K", type=int, default=60)  # top-k
